...
barbersRouter.get("/api/manage/barbers", requireAuth, requireOwner, async (req, res) => {
  ...
  res.json(barbers.map((b) => ({ ...toApiBarber(b), username: b.users[0]?.username ?? null })));
});
...
barbersRouter.put("/api/manage/barbers/:id", requireAuth, requireOwner, async (req, res, next) => {
  try {
    ...
    // existir), os dois juntos são obrigatórios pra criar uma agora.
    const existingUser = await getBarberUser(barberId);
    const trimmedUsername = username ? String(username).trim().toLowerCase() : undefined;
    if (trimmedUsername) await assertUsernameAvailable(trimmedUsername, existingUser?.id);
    if (password && String(password).length < 8) throw new AppError("A senha precisa ter pelo menos 8 caracteres");
    ...
    if (existingUser) {
      ...
      await createBarberUser(req.session.user!.barbershopId, barberId, trimmedName, trimmedUsername, hashPassword(String(password)));
    }
